...
namespace WebQQRobot
{
    public class Core
    {
        ...
        public static string getHash()
        {
            ...
            // 编译
            CompilerResults results = _provider.CompileAssemblyFromSource(parameters, funStr);
                    ...
                    case "group_message":
                        {
                            ...
                            if(str.IndexOf("@" + QQData["qq_name"]) != -1)
                            {
                                string msgStr = str.Replace("@" + QQData["qq_name"] + " ", "");
                                foreach(QQGroupList.Gnamelist2 g in GroupList.Result.Gnamelist)
                                {
                                    if (g.Code.ToString() == r.Value.GroupCode.ToString())
                                    {

                                        if (string.IsNullOrEmpty(msgStr.Trim()))
                                        {
                                            msgStr = "呜呜~你在说什么";
                                        }
                                        else
                                        {
                                            Match lol = Regex.Match(msgStr, @"ID【(.*?)】大区【(.*?)】");
                                            if (lol.Groups.Count > 0)
                                            if (!string.IsNullOrEmpty(lol.Groups[0].Value))
                                            {
                                                string name = Regex.Replace(Regex.Match(msgStr, @"ID【(.*?)】大区【(.*?)】").Groups[0].Value, "大区【(.*?)】", "").Replace("ID【", "").Replace("】", "");
                                                string region = Regex.Replace(Regex.Match(msgStr, @"ID【(.*?)】大区【(.*?)】").Groups[0].Value, "ID【(.*?)】", "").Replace("大区【", "").Replace("】", "");
                                                string info = LOLHelper.GetRank(name, region);
                                                if(string.IsNullOrEmpty(info))
                                                {
                                                    info = "对不起的喵，可能是主人输入的ID名不存在或者是大区不正确 T^T";
                                                }

                                                QQGroupSendMsg(info, g.Gid.ToString(), (r.Value.MsgId + 1).ToString());

                                                return;
                                            }

                                            Match q = Regex.Match(msgStr, @"问【(.*?)】答【(.*?)】");
                                            if(q.Groups.Count > 0)
                                            {
                                                // 加入问答
                                                if(q.Groups[0].Value.Trim() != "")
                                                {
                                                    string sql = @"INSERT INTO question_answer_library(question, answer, insert_datetime)
VALUES(@question, @answer, NOW())";
                                                    string question = Regex.Replace(Regex.Match(msgStr, @"问【(.*?)】答【(.*?)】").Groups[0].Value, "答【(.*?)】", "").Replace("问【", "").Replace("】", "");

                                                    string answer = Regex.Replace(Regex.Match(msgStr, @"问【(.*?)】答【(.*?)】").Groups[0].Value, "问【(.*?)】", "").Replace("答【", "").Replace("】", "");
                                                    if(0 < SQLHelper.ExecuteSQL(sql, question, answer))
                                                    {
                                                        QQGroupSendMsg("主人，我记住了喵~~", g.Gid.ToString(), (r.Value.MsgId + 1).ToString());
                                                    }
                                                    else
                                                    {
                                                        QQGroupSendMsg("呜呜对不起主人，我忘了刚才你说什么 T^T", g.Gid.ToString(), (r.Value.MsgId + 1).ToString());
                                                    }
                                                ...
                                                {
                                                    // 查询答案
                                                    string sql = @"SELECT * FROM question_answer_library	
                                                  WHERE question like CONCAT('%', @question,'%')
	                                              ORDER BY weight ASC
	                                              LIMIT 1";
                                                    DataTable dt = new DataTable();
                                                    dt = SQLHelper.QuerySQL(sql, msgStr.Trim());
                                                    
                                                    if (dt.Rows.Count <= 0)
                                                    {
                                                        QQGroupSendMsg("虽然不知道你在说什么，但是感觉好厉害的喵 0.0", g.Gid.ToString(), (r.Value.MsgId + 1).ToString());
                                                    }
                                                    else
                                                    {
                                                        string update = @"UPDATE question_answer_library SET weight = weight + 1
WHERE question like CONCAT('%', @question,'%')
ORDER BY weight ASC
LIMIT 1";
                                                        SQLHelper.ExecuteSQL(update, msgStr.Trim());
                                                        string answer = dt.Rows[0]["answer"].ToString();
                                                        QQGroupSendMsg(answer, g.Gid.ToString(), (r.Value.MsgId + 1).ToString());
                                                    }
                                                    return;
                                        ...
                                        // QQGroupSendMsg(msgStr, g.Gid.ToString(), (r.Value.MsgId + 1).ToString());
                                    }
                                }
                                // QQGroupSendMsg(msgStr, r.Value.)
                            }
                        }
                        break;
                    default: break;
